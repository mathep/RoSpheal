...
if (process.platform === "win32") {
	console.assert(
		(
			await Bun.$`tar -a -c --exclude ".DS_Store" --exclude "__MACOSX" --exclude "*_secret*" --exclude "*_dev*" --exclude ".git" -f ./builds-dist/RoSeal-${manifest.version}-src.zip "./cachedBuildTime.json" "./src" "./.npmrc" "./scripts" "./package.json" "./biome.json" "./tsconfig.json" "./bun.lock" "./README.md" "LICENSE"`
		).exitCode === 0,
	);
} else {
	console.assert(
		(
			await Bun.$`zip -r ./builds-dist/RoSeal-${manifest.version}-src.zip "./cachedBuildTime.json" "./src/"  "./.npmrc" "./scripts/" "./package.json" "./biome.json" "./tsconfig.json" "./bun.lock" "./README.md" "LICENSE" -x "**/.DS_Store" -x "**/__MACOSX" -x "**/*_secret*" -x "**/*_dev*" -x ".git" -9 > /dev/null`
		).exitCode === 0,
	);
}
